...
function StartRace () {
	...
	if(type == RaceStyle.Online)
	{
		...
		{
			Racers[i] = NetworkRacers[i].racer;
		}
	...
	else
	{
		if(type == RaceStyle.Online)
		{
			...
			{
				if(NetworkRacers[i].networkplayer.guid != networkView.owner.guid)
				networkView.RPC("SpawnMyKart",NetworkRacers[i].networkplayer);
				else
				transform.GetComponent(RaceBase).SpawnMyKart();
			}
			
			networkView.RPC("PlayCutscene",RPCMode.All);
...
function SendPositionUpdates()
{
	...
	{
		if(NetworkRacers[i].networkplayer.guid != networkView.owner.guid)
			networkView.RPC("SetPosition",NetworkRacers[i].networkplayer,Racers[i].position);	
		else
...
function EndRace()
{
	...
	{
		//Send Timer
		networkView.RPC("RaceEnded",RPCMode.AllBuffered);
	...
	if(toChange == -1 || NetworkRacers[toChange].networkplayer.guid != info.sender.guid)
		{
			...
			networkView.RPC("DisconnectMessage",info.sender,"You have been kicked for Cheating!");
